...
# hand-verified `# verified-for:` marker, instead of discovering it only
# after a ~22-minute devcontainer image build reaches `sha256sum --check`
# (harmon-init#1338).
#
...
# touches the hashes themselves, and it cannot (without re-downloading and
# re-hashing the release) prove that a moved marker was backed by an
# actually-redone verification. That trust boundary is the PR review, same
# as any other hand-verified pin; Renovate itself never touches this marker,
# since the hash ARGs carry no Renovate annotation.
set -euo pipefail
...
intentionally not Renovate-managed. Before merging this bump:
  1. Read the release notes and provenance: https://github.com/can1357/oh-my-pi/releases/tag/v${version}
