...
#
# Root variant: this is the harmon-init-specific twin-manifest script (it
# handles the root .skills-sync.yaml AND the template/ twin, validates both
# pins agree, and includes the full template-render verification battery).
# Generated repos render a simpler pull-based twin from template/
# (single manifest, no copier interactions); both share the same name and
# the same subcommands. See docs/architecture/ci-cd.md.
#
# Usage:
...
# The token is written to a 600 temp file so it never appears in /proc/*/cmdline
# (argv), and scrubbed from the git subprocess environment so hooks (which
# execute repository-controlled Taskfile targets) cannot inherit it. The sync
# and verification subprocesses run with GH_TOKEN scrubbed separately (see
# run_untrusted); git commit is also wrapped with env -u (line 660). The empty
