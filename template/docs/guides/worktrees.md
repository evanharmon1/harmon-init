...
working tree — yours included. This guide is the decision rule for _when_ a
...
1. **Will this work change tracked files and need to be committed?** Two
   ...
   heuristic — per unit of _committable_ work.
2. **Will it run repo tooling that assumes a private, stable tree?**
   ...
   mid-edit you get phantom failures. Note what a worktree does _not_
...
3. **Will it outlive a single turn, or need to be resumable?** A worktree is
   durable, _addressable_ state: if the session dies, the branch and its
...
If all three are **no**, you do not need a worktree. Work that _reads_ the
repo and _writes somewhere else_ — auditing, reviewing, research, reports,
...
writes `.terraform/`, builds write caches) count as _running the repo's
tooling_, question 2 — not as pane jobs in a shared checkout.
...
- **Serial work on an idle checkout does not need one.** One change at a time
  ...
  ceremony.
- **Worktree is not workspace.** The worktree decides _where_ a session's cwd
...
dependencies, proves the git hooks fire inside it (`.git` is a _file_ in a
...
is based on the _main worktree's_ HEAD — first verified against that branch's
configured upstream, so a merely _stale_ main is upgraded to the fresh
upstream tip (announced) while a _diverged_ one refuses with a `--base`
...
clears gitlink debris. Ignored _directories_ (`node_modules/`, `.venv/`,
