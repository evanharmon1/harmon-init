...
```text
preflight/claim → implement → task verify → task security → task challenge
  → task review → create DRAFT PR → shepherd the draft (CI, deferred findings,
...
These tasks slot into the **Dev Loop** above: after `task verify` and `task
security` go green, before the draft PR — and, where the skill's supported topology holds (`origin`
