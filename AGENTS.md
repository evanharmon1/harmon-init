...
test:template` will not catch it (it validates the _rendered_ template against
...
legitimately diverges in prose — so the gate is narrowed to _structure_: if the
...
of depth — but it still bounds a different thing: _other people's_ findings
...
_every_ parameter — every rigor level, review policy, budget envelope,
...
(attribution to _some_ actor is not authorization). **Unattended automation**
...
- **`task challenge`** — adversarial second-model review, under the resolved
  **challenge cap**. `/gauntlet` is the procedure **where its supported
  ...
  second such round: the second round _is_ the confirmation, so there is no
  ...
  rounds _are_ two rounds regardless of any floor, and a capped final round runs
  ...
  **Each adjudicated round, before the draft PR exists, ends in one
  ...
  _round_, not per finding: five fixes are one commit, and a round adjudicated
  ...
  challenge`/`task review` covers branch commits _and_ working tree alike, so
  ...
  **Push to the branch's own writable remote** — the one `gh pr create` will
  ...
  lost environment costs at most the current round's _code_ rather than every
  round's, and
...
- **Git transport** — pushes authenticate over HTTPS via `gh`. Provisioned hosts
  ...
  and the rewrite against the _named_ remote:
...
- **Shepherd the draft to ready for review (`/shepherd`, under the resolved
  ...
  **Do not hand-roll the polling.** Run the vendored
  ...
  flag the fix your _last_ round pushed, so if the rounds start circling your
  ...
  **a decision to stop one loop does not transfer to another**,
  ...
  to stop at it is unchanged: it bounds _other people's_ findings, not your
...
- **Checks green is a non-terminal state.** Reporting "all checks pass"
  ...
  the middle of the shepherd stage. Bot and human reviews land _after_ checks
...
- **Stop at ready-for-review.** When the readiness gate below passes, promote
  ...
  on the same head, report, and stop. Human approval is deliberately _not_ a
...
- Required CI checks have concluded successfully. An empty check list is
  _indeterminate_, not a pass — GitHub populates it asynchronously, so a read
...
- **Agents never merge to main** — no `gh pr merge`, `git merge`, or push to
  ...
  commands.) `gh pr ready` is _not_ a merge and agents may run it — but only
...
off a fresh asynchronous review _after_ the gate that was supposed to complete
...
_per-finding_, so a reviewer can be right every round while the loop as a whole
...
round's fixes, ask where they _live_ — in the change you set out to make, or in
...
in the adjudication table _and_ in the message of the commit that removes the
...
accreted procedure-prose _cannot_ simply be dropped because earlier rounds
...
One endpoint is worth knowing: if the deletion empties the change _entirely_,
...
whatever reviewer produced it. The sidecar records what is _deferred_, so an entry is
