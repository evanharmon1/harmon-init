...
| `task status:creds` | Credential logins (gh, Codex, Claude Code) + the gh token's scopes — local probes plus one bounded 3s scope check, and in the bot devcontainer profile a parallel bounded gh-identity check (`STATUS_NO_NETWORK=1` skips both); also run at session start |
