...
- [ ] **Foreman operator setup** — provision the separate READ-ONLY PAT that
      ...
      `… Immutability.json`, same UI import as the branch ruleset) and create
      the standing probe tag (`git tag v0.0.0-probe && git push origin
      v0.0.0-probe`) — preflight empirically asserts `v*` tags are immutable
      and fails until both exist. Then `task foreman:preflight` (inside the
