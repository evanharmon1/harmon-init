...
- [ ] **Upgrading from a release before harmon-init#1047
      (`method:*` → `strategy:*`)?** Rename every live `method:*` label to
      its `strategy:*` counterpart **before** either `task setup:github-labels`
      step below, not after: provisioning creates the `strategy:*`
      destinations first, and `gh label edit --name` refuses to rename onto a
      name that already exists — renaming afterward is rejected, stranding
      existing issues on the retired `method:*` label, which the resolver
      ignores. Start with an explicit `--limit 1000` on `gh label list` and
      treat an exactly-full result as capped (double the limit and re-run
      until the count is below the cap before renaming anything) — the same
      caution as the `agent:*` cleanup below. For each live value among
      `oneshot`, `plan`, `plan-approved`, `orchestrate`, `council`,
      `human-led`, run `gh label edit method:<v> --name strategy:<v>`
      (association-preserving: every issue keeps the label, just under its
      new name).
