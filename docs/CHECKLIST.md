...
- [ ] **Upgrading from a release before harmon-init#1047
      ...
      associations for matching issues and PRs found in its paginated snapshot,
...
- [ ] **[human-only] Inspect live label drift before retiring anything** — keep
      ...
      live labels and all-state issues and pull requests, and reports separate
      issue/PR association counts. An indeterminate read is a stop, not
...
- [ ] **[human-only] Retire any legacy `agent:*` claim labels or pre-2026
      ...
      `agent:claude-code` → `claim:claude`, `agent:gemini-cli` →
      `claim:gemini`, `agent:kimi-k2` → `claim:kimi`, `agent:qwen-code` →
      ...
      than replace their family-level label.

      If a destination already exists — normally because setup provisioned it —
      use this association-migration path, not `gh label edit` and not
      create-then-delete: the command validates a live registry destination,
      attempts the association move for each matching issue and PR found in its
      paginated snapshot, then permits `--prune` only when a fresh snapshot
      ...
      The command re-reads before removals and before each deletion and fails
      closed on read/verification errors, but GitHub has no transaction or
