...
- [ ] CI GitHub App: install the shared CI App on this repo, then set
      `CI_APP_ID` (Actions **variable**) + `CI_APP_PRIVATE_KEY` (Actions
      **secret**). Drives release-please, the claude-* workflows, and
      project-automation. Org repos: set these once as org-level
      variable/secret. See docs/security.md for the App + required permissions.
