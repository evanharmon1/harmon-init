...
version directly, it becomes a network-free no-op and leaves both files
absent instead, so `agy` resolves straight to the system binary (the bot's
`apply` still installs its flag-injecting wrapper over whichever of those
two states this installer leaves). It is gated on the rendered
