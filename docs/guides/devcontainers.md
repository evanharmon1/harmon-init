...
a Ghostty release changes the entry, since nothing here notices on its own. Like the Claude policy above it lives
...
a `TERM=… docker exec` prefix changes only the client's environment. Ask for the terminal you want with
