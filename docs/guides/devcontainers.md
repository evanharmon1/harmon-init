...
Nothing here is data loss by design for what already has a volume: shell
history and zoxide data sit on named volumes precisely so a recreation is
survivable, and so does each already-wired harness's state (Claude Code,
Codex, Antigravity, OpenCode) — `~/.claude.json` is symlinked onto one for the
same reason (below). Pi, oh-my-pi, and Copilot CLI are newer additions to the
image with no volume of their own yet, so their session/auth state does
**not** survive a recreation the way the others' does. What is otherwise lost
