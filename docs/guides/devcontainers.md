...
The human profile sets `GH_BROWSER` to a small host-browser bridge. When the VS
Code `code` command is available, GitHub's device page opens in the operator's
desktop browser via `code --open-url`. On Coder, the plain CLI, or a disconnected
VS Code session where that handoff is unavailable, the bridge prints the exact
URL to open manually. It deliberately never tries `w3m`, `lynx`, or generic
browser discovery, so an installed terminal browser cannot capture the flow.
Both the initial `gh auth login --web` and the browser flow used by
`task setup:gh-scopes` inherit this behavior from `GH_BROWSER`.
