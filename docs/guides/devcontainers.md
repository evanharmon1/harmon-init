...
- **An `unsupported` entry**, naming a reason and the executable `verify`
  ...
  it was unsupported. `copilot-cli` and `pi` are registered but not yet
  installed; `harness-matrix` installs them and a follow-on change adds their
  modules.
...
- **Bot profile — full autonomy.** It applies `always-proceed`, always accepts
  ...
  `--dangerously-skip-permissions` for agent runs. `devcontainer.json`'s
  `containerEnv.PATH` puts `~/.local/bin` ahead of the system binary
  container-wide, so the wrapper's precedence holds for a `docker exec` or any
  other programmatic launcher too — not only interactive shells. A launcher
  that resolves `agy` some other way must still pass that flag itself.
