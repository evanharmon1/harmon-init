...
`herdr` CLI. This guide is how to _use_ it for real work — the mental model,
...
| **Agent** | The coding agent Herdr recognizes _occupying_ a pane | pane ID or a unique name | `herdr agent start NAME --kind KIND --pane ID` |
...
- **A pane exists whether or not it hosts an agent.** `pane run` drives an
  ordinary shell; `agent start` requires an _available_ shell pane (at its
...
- **Two kinds of workspace.** `workspace create` makes a plain
  ...
  git worktree _and_ a workspace rooted at it, and records the binding; one
...
- `idle` — ready for input, and its tab has been _seen_ in the focused UI.
...
tabs for parallel _contexts_ (a second agent on a different task, a shell for
...
that already exists counts. Wait on a marker _you_ append (unique per run, one
...
`worktree open` binds an _existing_ checkout to a new workspace; `worktree
...
- A **subagent** (Claude Code's `Agent` tool, `.claude/agents/*.md`) runs
  _inside_ the parent session's process: its own context window, no terminal,
...
3. **Prompt** — one self-contained brief per worker, ending with a
   **file-based report** at a known path and a **sentinel line** printed
   last. Make both unique per _attempt_, not just per worker — a nonce such
...
4. **Wait** — `agent wait <name> --timeout <ms>` per worker (background it;
   ...
   waiting again blind. `done`/`idle` means _stopped_, not _succeeded_: the
...
subscription credentials _inside a different product_: Anthropic's consumer
...
- **`claude`** — `-- --model <m> --tools '<exact list>' --allowedTools '<the
  ...
  `--tools` restricts which tools _exist_ in the session, `--allowedTools`
...
- **`codex`** — `-- -a never -s workspace-write -c sandbox_workspace_write.network_access=true`
  ...
  matches _outside_ the sandbox, so blanket-allowing the task's whole command
