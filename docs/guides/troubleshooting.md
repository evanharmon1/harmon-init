...
- **"unknown terminal type" / broken keys in `vim`, `less`, or `htop`** — you are running Ghostty (`TERM=xterm-ghostty`) somewhere that lacks that terminfo entry. The container itself has it; SSH and `docker exec` into other containers do not — see [Terminal type](devcontainers.md#terminal-type-and-ghostty-terminfo).
