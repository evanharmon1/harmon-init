...
**Verified against:** commit `8c91172`, 2026-08-16. Counts below
...
| Kind | Count |
|---|---|
| Questions in `copier.yml` (asked + hidden) | **57** |
| — asked | **37** |
...
| 27 | `use_antigravity_cli` | bool | no | `devcontainer` | Prompt-free Google Antigravity CLI in the bot profile — **validator** |
| 28 | `use_alternative_claude_providers` | bool | no | `devcontainer` | `claude-kimi`/`-deepseek`/`-glm` wrappers; routes **paid** keys into *both* profiles |
| 29 | `project_management` | choice `none`/`github`/`linear` | `none` | always | `docs/project-management.md`, `setup:github-project` / `-labels`, `close-milestone-on-release.yml` |
| 30 | `git_init` | bool | **yes** | always | `_tasks`: `git init` + scaffold commit (copy only); also the fresh-scaffold signal for the other side effects |
| 31 | `github_remote_create` | bool | no | always | `_tasks`: `gh repo create --private --push` (copy only) |
| 32 | `github_release_init` | bool | no | always | `_tasks`: `task release:init` (copy only) |
| 33 | `bunch_add` | bool | no | always | `.meta/*.bunch` + `_tasks`: `task util:bunch-install` (macOS) |
| 34 | `bunches_directory` | str | `~/Library/Mobile Documents/com~apple~CloudDocs/Bunches` | `bunch_add` | Bunch install destination — **validator** (shell-unsafe characters) |
| 35 | `obsidian_project_add` | bool | no | always | `.meta/<project>.md` + `_tasks`: `task util:obsidian-install` (macOS) |
| 36 | `obsidian_directory` | str | `~/Local/Memex/Professional` | `obsidian_project_add` | Vault destination — **validator** (shell-unsafe characters) |
| 37 | `run_task_install` | bool | no | always | `_tasks`: `task install` — **ungated on `_copier_operation`, so it re-runs on every update** |

### Conditionally asked (10)

`deploy_cloudflare_workers`, `release_content_paths`, `skill_categories`,
`use_shared_agents`, `foreman_additional_trusted_actors`,
`use_codex_cloud_review`, `use_antigravity_cli`,
...
awk '/^[a-z][a-z_]*:/{n=$1} /^[[:space:]]+when: false/{print n}' copier.yml \
  | sort > "$hidden"
grep -oE '^[a-z][a-z_]*:' copier.yml | sort | comm -23 - "$hidden"     # 37 names
rm -f "$hidden"
