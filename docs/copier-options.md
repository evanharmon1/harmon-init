...
**Verified against:** commit `02f5ac0`, 2026-08-29. Counts below
...
> (v4.25.1) → 57/20 (#850) → 58/20 → 59/20 (#1042).
...
| 34 | `github_release_init` | bool | no | always | `_tasks`: `task release:init` (copy only) |
| 35 | `bunch_add` | bool | no | always | `.meta/*.bunch` + `_tasks`: `task util:bunch-install` (macOS) |
| 36 | `bunches_directory` | str | `~/Library/Mobile Documents/com~apple~CloudDocs/Bunches` | `bunch_add` | Bunch install destination — **validator** (shell-unsafe characters) |
| 37 | `obsidian_project_add` | bool | no | always | `.meta/<project>.md` + `_tasks`: `task util:obsidian-install` (macOS) |
| 38 | `obsidian_directory` | str | `~/Local/Memex/Professional` | `obsidian_project_add` | Vault destination — **validator** (shell-unsafe characters) |
| 39 | `run_task_install` | bool | no | always | `_tasks`: `task install` — **ungated on `_copier_operation`, so it re-runs on every update** |
| 40 | `decisions_seed_date` | str | today (`'%Y-%m-%d' \| strftime`) | always | Names and dates the seed decision record `docs/decisions/<date>-record-architecture-decisions.md` (and its README link) — **validator** (real calendar date); recorded once, so updates never rename the seed |

### Conditionally asked (12)
...
`use_alternative_claude_providers`, `use_statusline_pr_lookup`, `bunches_directory`,
`obsidian_directory`.
...
grep -cE '^[a-z][a-z_]*:' copier.yml                    # 60
...
awk '/^[a-z][a-z_]*:/{n=$1} /^[[:space:]]+when: false/{print n}' copier.yml \
  | sort > "$hidden"
grep -oE '^[a-z][a-z_]*:' copier.yml | sort | comm -23 - "$hidden"     # 40 names
rm -f "$hidden"
