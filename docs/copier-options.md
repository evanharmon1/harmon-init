...
**Verified against:** tag `v4.25.1`, commit `53408c1`, 2026-08-10. Counts below
...
| Kind | Count |
|---|---|
| Questions in `copier.yml` (asked + hidden) | **56** |
| — asked | **36** |
| — hidden (`when: false`) | **20** |
| Template-level `_`-prefixed keys | **7** |
| Distinct Jinja path-gate conditions under `template/` | **28** (over 82 gated paths, 4 of them directories) |
...
| 19 | `use_foreman` | bool | **no** | always | `taskfiles/`, `.foreman.toml`, tag-protection rulesets, `audit-foreman-adapters.sh`; **requires `devcontainer`** (validator on Q25) |
| 20 | `use_codeql` | bool | `project_type in ['web-astro','web-app']` | always | `codeql.yml` — free on public repos; private needs Code Security + `FULL_SECURITY_SCAN` |
| 21 | `codeql_languages` | multiselect `javascript-typescript`/`python` | seeded; `[]` when `use_codeql` off | always | CodeQL matrix — **validator**: ≥1 required when `use_codeql` |
| 22 | `use_codex_review` | bool | no | always | `docs/guides/codex-review.md`, `codex-review.sh`, `codex-gate.sh`, `task challenge` / `task review` |
| 23 | `use_codex_cloud_review` | bool | no | `use_codex_review` | Makes a terminal Codex review a **required shepherd signal** — **validator** |
| 24 | `use_coderabbit` | bool | no | always | `.coderabbit.yaml` + bot trust |
| 25 | `devcontainer` | bool | **yes** | always | The whole dual-profile `.devcontainer/`, `devcontainer-build.yml`, 3 scripts, 4 guides — **validator** rejects `use_foreman` without it |
| 26 | `use_antigravity_cli` | bool | no | `devcontainer` | Prompt-free Google Antigravity CLI in the bot profile — **validator** |
| 27 | `use_alternative_claude_providers` | bool | no | `devcontainer` | `claude-kimi`/`-deepseek`/`-glm` wrappers; routes **paid** keys into *both* profiles |
| 28 | `project_management` | choice `none`/`github`/`linear` | `none` | always | `docs/project-management.md`, `setup:github-project` / `-labels`, `close-milestone-on-release.yml` |
| 29 | `git_init` | bool | **yes** | always | `_tasks`: `git init` + scaffold commit (copy only); also the fresh-scaffold signal for the other side effects |
| 30 | `github_remote_create` | bool | no | always | `_tasks`: `gh repo create --private --push` (copy only) |
| 31 | `github_release_init` | bool | no | always | `_tasks`: `task release:init` (copy only) |
| 32 | `bunch_add` | bool | no | always | `.meta/*.bunch` + `_tasks`: `task util:bunch-install` (macOS) |
| 33 | `bunches_directory` | str | `~/Library/Mobile Documents/com~apple~CloudDocs/Bunches` | `bunch_add` | Bunch install destination — **validator** (shell-unsafe characters) |
| 34 | `obsidian_project_add` | bool | no | always | `.meta/<project>.md` + `_tasks`: `task util:obsidian-install` (macOS) |
| 35 | `obsidian_directory` | str | `~/Local/Memex/Professional` | `obsidian_project_add` | Vault destination — **validator** (shell-unsafe characters) |
| 36 | `run_task_install` | bool | no | always | `_tasks`: `task install` — **ungated on `_copier_operation`, so it re-runs on every update** |

### Conditionally asked (9)

`deploy_cloudflare_workers`, `release_content_paths`, `skill_categories`,
`use_shared_agents`, `use_codex_cloud_review`, `use_antigravity_cli`,
`use_alternative_claude_providers`, `bunches_directory`, `obsidian_directory`.
...
| Repo | project_type | foreman | codex_review | codex_cloud | coderabbit | skills_sync | skill_categories | shared_agents | codeql | codeql_languages | devcontainer | alt_providers | project_mgmt | snyk |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| harmon-init *(dogfood)* | general | ✅ | ✅ | ✅ | ❌ | ✅ | universal, repo | ✅ | ❌ | — | ✅ | ✅ | github | weekly |
...
(sommerlawn/sommerlawn-zoho#67). `n/a` = a `when:` condition excludes the
question.
...
grep -cE '^[a-z][a-z_]*:' copier.yml                    # 56
