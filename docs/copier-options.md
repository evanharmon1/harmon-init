...
**Verified against:** commit `600ddc8`, 2026-08-18. Counts below
...
| Kind | Count |
|---|---|
| Questions in `copier.yml` (asked + hidden) | **58** |
| — asked | **38** |
...
| 30 | `project_management` | choice `none`/`github`/`linear` | `none` | always | `docs/project-management.md`, `setup:github-project` / `-labels`, `close-milestone-on-release.yml` |
| 31 | `git_init` | bool | **yes** | always | `_tasks`: `git init` + scaffold commit (copy only); also the fresh-scaffold signal for the other side effects |
| 32 | `github_remote_create` | bool | no | always | `_tasks`: `gh repo create --private --push` (copy only) |
| 33 | `github_release_init` | bool | no | always | `_tasks`: `task release:init` (copy only) |
| 34 | `bunch_add` | bool | no | always | `.meta/*.bunch` + `_tasks`: `task util:bunch-install` (macOS) |
| 35 | `bunches_directory` | str | `~/Library/Mobile Documents/com~apple~CloudDocs/Bunches` | `bunch_add` | Bunch install destination — **validator** (shell-unsafe characters) |
| 36 | `obsidian_project_add` | bool | no | always | `.meta/<project>.md` + `_tasks`: `task util:obsidian-install` (macOS) |
| 37 | `obsidian_directory` | str | `~/Local/Memex/Professional` | `obsidian_project_add` | Vault destination — **validator** (shell-unsafe characters) |
| 38 | `run_task_install` | bool | no | always | `_tasks`: `task install` — **ungated on `_copier_operation`, so it re-runs on every update** |

### Conditionally asked (11)
...
| `devcontainer_coder_folder_uri` | empty, or a captured `vscode-remote://dev-container+…@ssh-remote+…/workspaces/…` folder URI; rejects generic/local Dev Containers commands and incomplete non-captures |
...
awk '/^[a-z][a-z_]*:/{n=$1} /^[[:space:]]+when: false/{print n}' copier.yml \
  | sort > "$hidden"
grep -oE '^[a-z][a-z_]*:' copier.yml | sort | comm -23 - "$hidden"     # 38 names
rm -f "$hidden"
