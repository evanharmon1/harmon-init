...
**Verified against:** commit `b302a6e`, 2026-08-25. Counts below
...
| Kind | Count |
|---|---|
| Questions in `copier.yml` (asked + hidden) | **59** |
| — asked | **39** |
...
grep -cE '^[a-z][a-z_]*:' copier.yml                    # 59
...
awk '/^[a-z][a-z_]*:/{n=$1} /^[[:space:]]+when: false/{print n}' copier.yml \
  | sort > "$hidden"
grep -oE '^[a-z][a-z_]*:' copier.yml | sort | comm -23 - "$hidden"     # 39 names
rm -f "$hidden"
