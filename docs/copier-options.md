...
**Verified against:** merged commits `40b9574` and `65a002d`, 2026-08-30. Counts below
...
| Kind | Count |
|---|---|
| Questions in `copier.yml` (asked + hidden) | **61** |
| — asked | **41** |
...
grep -cE '^[a-z][a-z_]*:' copier.yml                    # 61
...
awk '/^[a-z][a-z_]*:/{n=$1} /^[[:space:]]+when: false/{print n}' copier.yml \
  | sort > "$hidden"
grep -oE '^[a-z][a-z_]*:' copier.yml | sort | comm -23 - "$hidden"     # 41 names
rm -f "$hidden"
