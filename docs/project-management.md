...
- **Work type** — `bug`, `feature`, `task`, `research`: the personal-account
  work-type surface the issue forms apply; org repos use native issue Type
  instead and no work-type label
- **Area** — `area:template`, `area:devcontainer`, `area:ci`, …: which codebase
  subsystem the work lives in. Area is the *solution* space, `domain:` the
  *problem* space, `layer:` the stack slice; at most one of each per issue
- **Tier** — `tier:{local,economy,standard,frontier,apex,adaptive}`: which
  model-routing stratum should work the issue — advisory, human-written, and
  inert until a consumer resolves it under its own trust model
- **Method** — `method:{oneshot,plan,plan-approved,orchestrate,council,human-led}`:
  the execution topology to work the issue under — advisory, like `tier:`
