...
view**). Each is a saved **filter + layout + grouping** over the one board:

| View | Layout | Filter | Group / sort |
| --- | --- | --- | --- |
| Board | Board | `is:open` | group by `Status` |
| Triage | Table | `is:open is:issue no:status` | sort by newest |
| Agent queue | Board | `is:open status:"Agent Queue"` | group by `Agent` |
| Priority | Table | `is:open` | sort by `Priority`, then `Effort` |
| Roadmap | Roadmap | `is:open` | lay out on `Start date` → `Target date` |
| Mine | Table | `is:open assignee:@me` | sort by `Priority` |

Roadmap needs the date fields (`Start date` / `Target date` are org issue-field
built-ins; add them as project fields on a personal account). Slice further by
`Product` or iteration (`iteration:@current`) rather than spinning up more
projects.
