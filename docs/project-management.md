...
| Issue closed as completed | **Done** | built-in workflow, gated on `state_reason` |
...
Release is symmetric and loud. An `always()` step releases the claim on every
terminal path — success, failure, step timeout, cancellation — but only when
*this* run acquired it, so a claim that was already there is never stolen. A
release that cannot be confirmed retries once and then turns the job **red**
with the marker still on the issue: a release reported as successful over a
surviving label would be permanent, because the next run would read the claim,
record that it did not acquire it, and never clean it either.
