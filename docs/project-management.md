...
by hand. (`feature` replaces GitHub's default `enhancement`, which is retired
from this vocabulary — rename the old label to keep its issue associations,
or prune it.) Keep the rest in a few families, color-coded by family; the starter
...
writable and visible to an agent holding nothing but repo scope, on personal
...
| GitHub's remaining defaults (`documentation`, `duplicate`, `enhancement`, `question`, …) | GitHub, at repo creation | humans | not provisioned, never deleted by setup | prune by hand if you do not want them; `enhancement` is retired in favor of `feature` — rename it to keep its issue associations, or prune it |
| `bug`, `feature`, `task`, `research` | humans, at triage — or a personal-account issue form, automatically | humans, `gh issue list --label` | provisioned — bug matches GitHub's own repo-creation color/description, so reconciling an unmodified repo is a no-op; feature keeps retired `enhancement`'s color, so a rename-migrated repo reconciles as a no-op too; task/research have no GitHub default | durable classification — the form-backed four of the six personal-account work types (`documentation`/`question`, the row above, are the other two: GitHub's own defaults, applied by hand); org repos set native issue `Type` instead and never write any of the six |
...
The `suggest:` / `claim:` vocabulary and the Foreman adapter selectors come
